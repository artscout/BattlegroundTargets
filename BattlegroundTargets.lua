...
local function BuildBattlegroundMapTable()
	for i = 2, GetNumBattlegroundTypes() do
		local localizedName, _, _, _, bgID, _, bgMapID, _, gameType, icon = GetBattlegroundInfo(i)
		--print(localizedName, bgID, gameType, icon, i, "#", GetBattlegroundInfo(i))
		--GetCurrentMapAreaID()
		if bgID     == 1   then mapID[91] = localizedName bgMaps[localizedName] = {bgSize = 40, flagBG = 0} -- Alterac Valley
		elseif bgID == 2   then mapID[92] = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 1} -- Warsong Gulch
		elseif bgID == 3   then mapID[93] = localizedName bgMaps[localizedName] = {bgSize = 15, flagBG = 0} -- Arathi Basin 1366
		elseif bgID == 7   then mapID[112] = localizedName bgMaps[localizedName] = {bgSize = 15, flagBG = 2} -- Eye of the Storm
		elseif bgID == 9   then mapID[128]  = localizedName bgMaps[localizedName] = {bgSize = 15, flagBG = 0} -- Strand of the Ancients
		elseif bgID == 30  then mapID[169]  = localizedName bgMaps[localizedName] = {bgSize = 40, flagBG = 0} -- Isle of Conquest
		elseif bgID == 108 then mapID[206]  = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 3} -- Twin Peaks
		elseif bgID == 120 then mapID[275]  = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 0} -- The Battle for Gilneas
		elseif bgID == 699 then mapID[417] = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 4} -- Temple of Kotmogu 417, 449
		elseif bgID == 708 then mapID[423] = localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 0} -- Silvershard Mines
		elseif bgID == 754 then mapID[519] = localizedName bgMaps[localizedName] = {bgSize = 15, flagBG = 5} -- Deepwind Gorge
...
--		elseif bgID == 894 then mapID[1186]= localizedName bgMaps[localizedName] = {bgSize = 10, flagBG = 0} -- Seething Shore
	...
	-- Add rated BattleGround maps
	RatedEOSLocalizedName = C_Map.GetMapInfo(397).name
	mapID[397] = RatedEOSLocalizedName
	if not bgMaps[RatedEOSLocalizedName] then
		bgMaps[RatedEOSLocalizedName] = {bgSize = 10, flagBG = 2}
	end
end
...
classROLES = {HEALER = {}, TANK = {}, DAMAGER = {}}

local classSpecs = {}
classSpecs["DEATHKNIGHT"] = {250, 251, 252}
classSpecs["DRUID"]       = {102, 103, 104, 105}
classSpecs["HUNTER"]      = {253, 254, 255}
classSpecs["MAGE"]        = {62, 63, 64}
classSpecs["MONK"]        = {268, 270, 269}
classSpecs["PALADIN"]     = {65, 66,70}
classSpecs["PRIEST"]      = {256, 257, 258}
classSpecs["ROGUE"]       = {259, 260, 261}
classSpecs["SHAMAN"]      = {262, 263, 264}
classSpecs["WARLOCK"]     = {265, 266, 267}
classSpecs["WARRIOR"]     = {71, 72, 73}

for classID = 1, 12 do -- druid 11
	...
	if classInfo then
		...
		for i = 1, numTabs do
			local specID = classSpecs[classTag][i]
			local id, name, _, icon, specRole, specClass = GetSpecializationInfoByID(specID)
			if specClass == classTag then
				if     specRole == "DAMAGER" then classes[classTag].spec[i] = {role = 3, specID = id, specName = name, icon = icon} tinsert(classROLES.DAMAGER, {classTag = classTag, specIndex = i}) -- DAMAGER: total = 23
				elseif specRole == "HEALER"  then classes[classTag].spec[i] = {role = 1, specID = id, specName = name, icon = icon} tinsert(classROLES.HEALER,  {classTag = classTag, specIndex = i}) -- HEALER : total =  6
				elseif specRole == "TANK"    then classes[classTag].spec[i] = {role = 2, specID = id, specName = name, icon = icon} tinsert(classROLES.TANK,    {classTag = classTag, specIndex = i}) -- TANK   : total =  5
				end
			end
...
function BattlegroundTargets:CreateInterfaceOptions()
	...
	GVAR.InterfaceOptions.CONFIG:SetScript("OnClick", function()
		if Settings then
			Settings.OpenToCategory(GVAR.InterfaceOptions.name)
		else
	...
	if Settings and Settings.RegisterCanvasLayoutCategory then
		local category;
		category = Settings.RegisterCanvasLayoutCategory(GVAR.InterfaceOptions, GVAR.InterfaceOptions.name)
...
function BattlegroundTargets:DefaultShuffle()
	for frc = 1, #FRAMES do
		...
		for i = 1, 40 do
			...
			local classToken = class_IntegerSort[ random(1,10) ].cid
			local talentSpec = classes[ classToken ].spec[ random(1,2) ].role
	...
	if not currentBGMap then
		...
		if bgMaps[bgName] then
			...
			if bgMaps[mapName] then
				...
				print(format("No map detected for BG: %s, mapID: %s", bgName, mapId))
			end
		end
	else
		if not bgMaps[currentBGMap] then
			return
		end
	end
...
function BattlegroundTargets:CheckUnitTarget(unitID, unitName, isEvent)
	...
	-- level
	if isLowLevel and playerLevel ~= 90 then -- LVLCHK
...
function BattlegroundTargets:CheckFlagCarrierCHECK(unitID, unitName) -- FLAGSPY
	if not DATA.FirstFlagCheck[unitName] then 
		return
	end
...
function BattlegroundTargets:Carrier_WG_TP(message, messageFaction)
	...
	if messageFaction == playerFactionBG then
		-- -------------------------------------------------------------------------
		local ffc = strmatch(message, FLG["WG_TP_DG_PATTERN_PICKED1"]) or -- Warsong Gulch & Twin Peaks: flag was picked
